<#
.SYNOPSIS
    DataPlatform-Automation - Storage Platform Loopback Mount Worker
.DESCRIPTION
    Actual ISO mount logic using the native, reliable Mount-DiskImage API.
    This script is only ever invoked elevated (via the SYSTEM-privileged
    scheduled task, see mount_iso.ps1 / invoke_elevated.ps1), so it does
    not need to check for Administrator rights itself.
    Enforces true idempotency, handles asynchronous PnP race conditions, and
    persists the drive letter for downstream automation contexts.
.NOTES
...
Write-Output "[INIT] Starting SQL Server ISO mount phase (elevated worker)..."
