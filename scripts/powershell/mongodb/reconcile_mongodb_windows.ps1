...
function Parse-Service([object]$service) {
 ...
 if($cp -and (Test-Path -LiteralPath $cp)){$lines=Get-Content -LiteralPath $cp;if(-not $port){$port=(($lines|Select-String '^\s*port\s*:\s*(\d+)'|Select-Object -First 1).Matches.Groups[1].Value)};if(-not $dp){$dp=(($lines|Select-String '^\s*dbPath\s*:\s*(.+?)\s*$'|Select-Object -First 1).Matches.Groups[1].Value).Trim(' ','"',"'")}}
 [pscustomobject]@{Executable=$exe;ConfigFile=$cp;Port=$port;DataPath=$dp}
}
...
if(-not(Test-Path -LiteralPath $mongod)){if($existing -and $existing.Executable -and(Test-Path -LiteralPath $existing.Executable)){throw "Configured MONGODB_EXECUTABLE is missing: $mongod. Existing deployment is $($existing.Executable). Update mongodb.conf to adopt it; binaries were not replaced."};Install-RequiredBinary (Required $s 'MONGODB_DOWNLOAD_URL') (Join-Path (Split-Path -Parent $installDir) 'downloads\mongodb.zip') $installDir $mongod}else{Log 'Existing mongod executable detected; reusing binaries.'}
...
if($existing -and $existing.DataPath -and (([IO.Path]::GetFullPath($existing.DataPath)).TrimEnd('\') -ne ([IO.Path]::GetFullPath($dataDir)).TrimEnd('\'))){throw "Configured data directory ($dataDir) differs from existing data directory ($($existing.DataPath)). Data-directory migration is blocked to preserve databases; update mongodb.conf or perform an explicit migration."}
