...
DATABASE = "postgresql"
ROLE_NAMES = {r: f"dp_{r}" for r in ("admin", "developer", "qa", "viewer")}


def connect(c, database=None, user=None, password=None):
    return psycopg2.connect(
        host=c["POSTGRESQL_HOST"],
        port=int(c["POSTGRESQL_PORT"]),
        database=database or c["POSTGRESQL_DB"],
        user=user or c["POSTGRESQL_USER"],
        password=password or c["POSTGRESQL_PASSWORD"],
    )


def configure():
    c, u = database_rbac_config(DATABASE)
    log = get_rbac_logger(DATABASE)

    if c.get("RBAC_ENABLED", "true").lower() == "false":
        log.info("RBAC disabled by configuration")
        return

    db = validate_identifier(c["POSTGRESQL_DB"], "database name")

    # --- Create base roles and per-user login roles, grant membership ---
    conn = connect(c, "postgres")
        ...
        for r, x in u.items():
            cur.execute(
                sql.SQL(
                    "DO $$ BEGIN IF NOT EXISTS "
                    "(SELECT 1 FROM pg_roles WHERE rolname={n}) "
                    "THEN CREATE ROLE {i} LOGIN PASSWORD {p}; "
                    "ELSE ALTER ROLE {i} LOGIN PASSWORD {p}; END IF; END $$"
                ).format(
                    n=sql.Literal(x["username"]),
                    i=sql.Identifier(x["username"]),
                    p=sql.Literal(x["password"]),
                )
            )
            cur.execute(
                sql.SQL("GRANT {} TO {}").format(
                    sql.Identifier(ROLE_NAMES[r]),
                    sql.Identifier(x["username"]),
                )
            )
            cur.execute(
    ...
    # --- Grant schema/table/sequence/function privileges per role ---
    conn = connect(c)
    conn.autocommit = True
    cur = conn.cursor()
    try:
        for role in ("admin", "developer", "qa", "viewer"):
            ...
            cur.execute(
                sql.SQL("GRANT USAGE ON SCHEMA public TO {}").format(
                    sql.Identifier(ROLE_NAMES[role])
                )
            )
        ...
        cur.execute(
            ...
            ).format(sql.Identifier(ROLE_NAMES["developer"]))
        )
...
def validate():
    ...
    def check(label, allow, role, statement):
        try:
            x = connect(c, user=u[role]["username"], password=u[role]["password"])
            x.autocommit = True
            q = x.cursor()
            q.execute(statement)
            q.close()
            ...
            actual = False
            print(f"{'PASS' if actual == allow else 'FAIL'} {label}: {e.__class__.__name__}")
        else:
            print(f"{'PASS' if actual == allow else 'FAIL'} {label}")
    ...
    x = connect(c, "postgres")
    x.autocommit = True
    q = x.cursor()
    q.execute("DROP DATABASE IF EXISTS dp_rbac_validation_tmp")
    q.close()
    x.close()
    ...
    q.execute(
        """
        GRANT SELECT ON TABLE public.dp_rbac_validation_tmp
        TO dp_qa, dp_viewer
        """
    )
    q.close()
    ...
    x.autocommit = True
    q = x.cursor()
    q.execute("DROP TABLE IF EXISTS public.dp_rbac_validation_tmp")
    q.close()
    x.close()
...
def main():
    a = command_arguments("PostgreSQL RBAC")
    configure()
    if a.command == "validate" or not a.skip_validation:
        validate()
