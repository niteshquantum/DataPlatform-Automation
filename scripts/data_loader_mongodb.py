...
def get_cdc_status(file_name):
    """
    Read CDC status generated by schema detector.
    """
    ...
    try:
        ...
        table_name = Path(file_name).stem.lower().replace(" ", "_")
        ...
        logger.warning(f"Unable to read CDC status: {e}")
        return "UNCHANGED"
...
def load_file_into_mongodb(db, path):
    ...
    cdc_status = get_cdc_status(path.name)

    if cdc_status == "CHANGED":
        logger.info(f"Schema changed. Dropping collection: {collection_name}")
        collection.drop()
        collection = db[collection_name]

    elif cdc_status == "DELETED":
        logger.info(f"Schema deleted. Recreating collection: {collection_name}")
        collection.drop()
        collection = db[collection_name]
        ...
        logger.info(f"Collection unchanged: {collection_name}")
...
def main():
    ...
    for path in data_files:
        ...
        if cdc_status == "UNCHANGED" and is_file_already_processed(path.name):
