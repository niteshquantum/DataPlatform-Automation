def context = [database: 'mysql', action: 'cleanup', operatingSystem: 'windows']
pipeline {
    ...
    post {
        success { echo 'WINDOWS MYSQL CLEANUP SUCCESSFUL' }
        failure { echo 'WINDOWS MYSQL CLEANUP FAILED' }
        always { script { load('jenkins/common/standalone_pipeline_support.groovy').finalize(context) }; echo 'WINDOWS MYSQL CLEANUP PIPELINE COMPLETED' }
    }
}
