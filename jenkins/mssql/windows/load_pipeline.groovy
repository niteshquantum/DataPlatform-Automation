def context = [database: 'mssql', action: 'load', operatingSystem: 'windows']
pipeline {
    ...
    post {
        success { echo 'WINDOWS MSSQL LOAD SUCCESSFUL' }
        failure { echo 'WINDOWS MSSQL LOAD FAILED' }
        always { script { load('jenkins/common/standalone_pipeline_support.groovy').finalize(context) }; echo 'WINDOWS MSSQL LOAD PIPELINE COMPLETED' }
    }
}
